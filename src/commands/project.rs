...
async fn handle_create(ctx: &Ctx, name: &str, key_prefix: &str) -> Result<()> {
    ...
    if ctx.format.is_json() {
        ...
        let msg =
            crate::format::inline::mutation_created("project", key, Some(&resp.name), &ctx.color);
        ...
        let rows: Vec<Vec<Option<String>>> = resp
            ...
            .map(|p| {
                vec![
                    Some(
                        ctx.color
                            .paint(project_key_or_prefix_item(p), colors::MUTED),
                    ),
                    Some(p.name.clone()),
...
async fn handle_view(ctx: &Ctx, key: &str) -> Result<()> {
    ...
    if ctx.format.is_json() {
        ...
        let fields = vec![
            ...
            CardField::new(
                "Status",
                Some(resp.status.clone().unwrap_or_else(|| "Active".to_string())),
            ),
        ...
        print!(
            "{}",
            render_card(display_key, &resp.name, &sections, &ctx.color)
        );
    }
    Ok(())
}
...
async fn handle_update(
    ...
    if let Some(k) = key_prefix {
        body["key_prefix"] = json!(k);
    }

    let resp: MutationResponse = ctx
        .api
        .patch(&format!("/v1/projects/{}", project.id), &body)
        .await?;
